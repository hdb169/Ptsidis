void weighted_diff(){
  ...
  const double kin[] = {3.898,4.75,4.764,5.5,4};
  const double xbj[] = {0.45,0.45,0.55,0.6, 0.35};
 ...
 const int nkin = sizeof(kin)/sizeof(*kin);
  ...
  for(int ikin=0;ikin<nkin;ikin++){
   ofstream outfile(Form("./TextFiles/weighted_average_%.2f.csv",kin[ikin]));
   for(int ix=0;ix<nx;ix++){
    double kinematics, x, y, err;
    vector<double>Kinematics, X, Y, Err;
    ifstream infile("./TextFiles/weighted_diff.csv");
    if(!infile){
 ...
 for(int ikin=0;ikin<nkin;ikin++){
  ifstream inf(Form("./TextFiles/weighted_average_%.2f.csv",kin[ikin]));
  double kk, xx, yy, ee;
 ...
 // TMultiGraph* mg = new TMultiGraph("mg","Weighted Y vs X;X;Y (weighted)");
  TMultiGraph* mg = new TMultiGraph("mg","Diff Ratio of H2 +/- and D2 +/- (CSV Data);Z; Diff Ratio");
for(int ikin=0;ikin<nkin;ikin++){
   mg->Add(gr1[ikin]);
 ...
 lg->Draw();
 c_multi->SaveAs("./plots/weighted_Diffratio_vs_z.pdf");
}
