...
std::vector<double> Get_weighted_averages(std::vector<int> RunNumbers,int bins){
  ...
  for(int i = 1;i<=bins;++i){
    ...
    for(auto it = RunNumbers.begin();it!=RunNumbers.end();++it){
      ...
      //can get charge
      TFile *root_file = new TFile(("/home/hdbhatt/yield/yield_root_20bins_hgcer_mx22p4_pt/count_histo_"+std::to_string(RunNumber)+".root").c_str());//all the data count hists
...
std::vector<double> Get_weighted_errs(std::vector<int> RunNumbers,int bins){
  ...
  for(int i = 1;i<=bins;++i){
    ...
    for(auto it = RunNumbers.begin();it!=RunNumbers.end();++it){
      int RunNumber = *it;
      double charge = j_info[(std::to_string(RunNumber)).c_str()]["ChargeCorr"].get<double>();
      TFile *root_file =new TFile(("/home/hdbhatt/yield/yield_root_20bins_hgcer_mx22p4_pt/count_histo_"+std::to_string(RunNumber)+".root").c_str());//counts hists
...
int testH2simc(){
  ...
  // ofstream txtsum("new500_txt_sum_500_d2overh2.txt",ios::app);
  // ofstream txtdiff("new500_txt_diff_500_d2overh2.txt",ios::app);

  // ofstream txtsum1("new500_check_sum.txt",ios::app);
  // ofstream txtdiff1("new500_check_diff.txt",ios::app);

  ofstream txtsum("april29_csv_d2overh2_sum.txt",ios::app);
  ofstream txtdiff("april29_csv_d2overh2_diff.txt",ios::app);

  ofstream txtsum1("relative_err_sum.txt",ios::app);
  ofstream txtdiff1("relative_err_diff.txt",ios::app);
  ...
  json j_Q2x;
  {
   ...
   std::ifstream runs("/home/hdbhatt/yieldnew/db2/kin_group_H2D2_csv_only.json");//has on D2-H2 rootfile json for Fact check
  ...
  for(json::iterator it = j_Q2x.begin();it!=j_Q2x.end();++it){
    ...
    for(json::iterator it  = j_Q2z.begin();it!=j_Q2z.end();++it){
	  ...
	  TFile *rootfile_pos_simH2 = new TFile(("/home/hdbhatt/yieldsimc2022/results/yield20binsptt/kinematics_yield_"+std::to_string(RunGroup)+"_simc.root").c_str());//H2
	  TFile *rootfile_pos_sim   = new TFile(("/home/hdbhatt/yieldsimc2022/results/yield20binsptt/kinematics_D2_yield_"+std::to_string(RunGroup)+"_simc.root").c_str());//D2
	  ...
	  h2_z_pos_sim_incnorad = (TH1D*)rootfile_pos_simH2->Get("z_pos_inc_norad");
          ...
          h2_z_pos_sim_excrad = (TH1D*)rootfile_pos_sim->Get("z_pos_exc_rad");
          TH1D *h2_z_pos_sim_rho = new TH1D("","pi+ sim rho",bins,0,1);
          h2_z_pos_sim_rho = (TH1D*)rootfile_pos_simH2->Get("z_pos_rho");
	  TH1D *h2_z_pos_sim_delta = new TH1D("","pi+ sim delta",bins,0,1);
	  h2_z_pos_sim_delta = (TH1D*)rootfile_pos_sim->Get("z_pos_delta");
          TH2D *h2_xs_z_pos_sim = new TH2D("","pi- sim xs",bins,0,1,bins,0,1);
          h2_xs_z_pos_sim = (TH2D*)rootfile_pos_sim->Get("xs_z_pos_inc_rad");
	  ...
	  std::cout<<"Now Working  5"<<std::endl;
          std::string ch2_radia_pos_name = "/u/group/c-csv/hdbhatt/yielddec21/yieldJan_2022/results/yield/statistics_corr/radiah2_corr_neg"+q2xz_str_filename+"_pos.pdf";
          ch2_radia_pos->SaveAs(ch2_radia_pos_name.c_str());
          TCanvas* ch2_radia_neg = new TCanvas();
          rph2_radia_corr_neg->Draw();
          std::string ch2_radia_neg_name = "/u/group/c-csv/hdbhatt/yielddec21/yieldJan_2022/results/yield/statistics_corr/radiah2_corr_neg"+q2xz_str_filename+"_neg.pdf";
	  ...
	  std::cout<<"Now Working  5"<<std::endl;
          std::string c_radia_pos_name = "/u/group/c-csv/hdbhatt/yielddec21/yieldJan_2022/results/yield/statistics_corr/radia_corr_neg"+q2xz_str_filename+"_pos.pdf";
          c_radia_pos->SaveAs(c_radia_pos_name.c_str());
          TCanvas* c_radia_neg = new TCanvas();
          rp_radia_corr_neg->Draw();
          std::string c_radia_neg_name = "/u/group/c-csv/hdbhatt/yielddec21/yieldJan_2022/results/yield/statistics_corr/radia_corr_neg"+q2xz_str_filename+"_neg.pdf";
	  ...
	  mg_z_yield_pos->Add(g_z_pos_sim_delta); 
          ...
          std::string c_yield_pos_name = "/u/group/c-csv/hdbhatt/yielddec21/yieldJan_2022/results/yield/statistics_corr/yield_"+q2xz_str_filename+"_pos.pdf";
	  ...
	  mg_z_yieldH2_pos->Add(g_z_pos_sim_delta); 
          ...
          std::string c_yieldH2_pos_name = "/u/group/c-csv/hdbhatt/yielddec21/yieldJan_2022/results/yield/statistics_corr/yieldH2_"+q2xz_str_filename+"_pos.pdf";
	  ...
	  int nbins_yield_neg = bins;
	    ...
	    //////add 8 done
          ...
          std::string c_yield_neg_name = "/u/group/c-csv/hdbhatt/yielddec21/yieldJan_2022/results/yield/statistics_corr/yield_"+q2xz_str_filename+"_neg.pdf";
  ...
  mg_z_yieldH2_neg->Add(gh2_z_neg_sim); ///============================================================Corrected
	  ...
	  mg_z_yieldH2_neg->Add(gh2_z_neg_sim_delta); 
          ...
          std::string c_yieldH2_neg_name = "/u/group/c-csv/hdbhatt/yielddec21/yieldJan_2022/results/yield/statistics_corr/yieldH2_"+q2xz_str_filename+"_neg.pdf";
	  ...
	  //add 10+1===========================================================================================================
	    ...
	    */
	    ...
	    double y_pos_exc_H2 = h2_z_pos_sim_excrad->GetBinContent(i+1);
            ...
            yH2_neg = yH2_neg*radia_corrH2_neg;
            yH2_pos = yH2_pos*radia_corrH2_pos;
            errorH2_pos = errorH2_pos*radia_corrH2_pos;
            errorH2_neg = errorH2_neg*radia_corrH2_neg;
	  ...
	  std::string zratiopdfname = "/u/group/c-csv/hdbhatt/yielddec21/yieldJan_2022/results/yield/statistics_corr/"+q2xz_str_filename+"_ratio.pdf";
	...
	sum_ratio->GetXaxis()->SetRangeUser(0.4,1);
	sum_ratio->GetYaxis()->SetRangeUser(0.2,1);
	...
	diff_ratio->GetXaxis()->SetRangeUser(0.4,1);
	diff_ratio->GetYaxis()->SetRangeUser(0.4,1.5);
	...
	gPad->Update();
	//cc->SaveAs("sum_diff.pdf");
	std::string sumname = "/u/group/c-csv/hdbhatt/yielddec21/yieldJan_2022/results/yield/statistics_corr/"+canvas_filename+"_sum_ratio.pdf";
	//c_Q2x_ratio->BuildLegend(0.6,0.6,0.95,0.95);
	cc->SaveAs(sumname.c_str());
	std::string diffname = "/u/group/c-csv/hdbhatt/yielddec21/yieldJan_2022/results/yield/statistics_corr/"+canvas_filename+"_diff_ratio.pdf";
	//c_Q2x_ratio->BuildLegend(0.6,0.6,0.95,0.95);
	...
	std::string ratiopdfname = "/u/group/c-csv/hdbhatt/yielddec21/yieldJan_2022/results/yield/statistics_corr/"+canvas_filename+"_ratio.pdf";
	//c_Q2x_ratio->BuildLegend(0.6,0.6,0.95,0.95);
	...
	std::string RDpdfname = "/u/group/c-csv/hdbhatt/yielddec21/yieldJan_2022/results/yield/statistics_corr/"+canvas_filename+"_RDratio.pdf";
